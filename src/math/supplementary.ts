export function supplementary(num: number, num2?: number):any {
		if(!num) return `Provide atleast 1 number`
		if(!num2 && num === 180) return `There is no second supplementarty number`
	if(!num2 && num < 180) {
		let res = 180 - num
